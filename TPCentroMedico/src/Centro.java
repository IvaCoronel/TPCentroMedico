...
public class Centro {
	...
	protected ArrayList<Integer> pacientesInternados;
	...
	//*******************************************************************************************************//
	protected Paciente obtenerPaciente(Integer hc){
		return pacientes.get(hc);
	}
	
	protected Medico obtenerMedico(Integer matricula){
	...
	// **********ATENCION DE TIPO GUARDIA****************//
	public void agregarAtencion(int hc, Fecha fecha) {
		Guardia g=new Guardia(fecha);
		Paciente p = obtenerPaciente(hc);
		if(p!=null && p instanceof PacientePrivado && !(p.atenciones.contains(g))) {
			p.atenciones.add(g);
		}
		else {
			throw new RuntimeException("El paciente es inválido o no es de tipo Privado");
	...
	public void agregarTratamiento(int hc, int matricula, String area){
		...
		if(p!=null && p instanceof PacienteAmbulatorio) {
				p.atenciones.add(t);
				double costo=(obtenerMedico(matricula).getValorHonorarios());
				t.setMonto(costo);
	...
	private String obtenerEspecialidad(Integer matricula){
		 Medico m=obtenerMedico(matricula);
